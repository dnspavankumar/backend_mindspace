...
router.get('/posts', async (req, res) => {
    try {
        ...
        const posts = await Forum_1.Post.find(query)
            .populate('comments')
            .sort({ timestamp: -1 })
            .lean();
...
router.delete('/posts/:id', async (req, res) => {
    try {
        ...
        const post = await Forum_1.Post.findById(id);
        if (!post) {
        ...
        await Forum_1.Comment.deleteMany({ postId: id });
        // Delete the post
        await Forum_1.Post.findByIdAndDelete(id);
...
router.get('/posts/:id/comments', async (req, res) => {
    try {
        ...
        const comments = await Forum_1.Comment.find({ postId: id })
            .sort({ timestamp: 1 })
            .lean();
...
router.post('/comments', async (req, res) => {
    try {
        ...
        const post = await Forum_1.Post.findById(postId);
        if (!post) {
            return res.status(404).json({ error: 'Post not found' });
        }
        // Generate avatar
        const avatar = isAnonymous ? 'AN' : author.substring(0, 2).toUpperCase();
        const comment = new Forum_1.Comment({
            content,
            author: isAnonymous ? 'Anonymous' : author,
            avatar,
            postId,
...
router.post('/comments/:id/like', async (req, res) => {
    try {
        ...
        const comment = await Forum_1.Comment.findById(id);
        if (!comment) {
